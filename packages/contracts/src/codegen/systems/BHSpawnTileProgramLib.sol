...
import { BHSpawnTileProgram } from "../../BHSpawnTileProgram.sol";
...
type BHSpawnTileProgramType is bytes32;

// equivalent to WorldResourceIdLib.encode({ typeId: RESOURCE_SYSTEM, namespace: "rgtmpff1", name: "BHSpawnTileProgr" }))
BHSpawnTileProgramType constant bHSpawnTileProgram = BHSpawnTileProgramType.wrap(
  0x73797267746d706666310000000000004248537061776e54696c6550726f6772
);
...
library BHSpawnTileProgramLib {
  ...
  function _msgValue(BHSpawnTileProgramType self) internal view returns (uint256 __auxRet0) {
  ...
  function callFrom(BHSpawnTileProgramType self, address from) internal pure returns (CallWrapper memory) {
    return CallWrapper(self.toResourceId(), from);
  }

  function callAsRoot(BHSpawnTileProgramType self) internal view returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), WorldContextConsumerLib._msgSender());
  }

  function callAsRootFrom(BHSpawnTileProgramType self, address from) internal pure returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), from);
  }

  function toResourceId(BHSpawnTileProgramType self) internal pure returns (ResourceId) {
    return ResourceId.wrap(BHSpawnTileProgramType.unwrap(self));
  }

  function fromResourceId(ResourceId resourceId) internal pure returns (BHSpawnTileProgramType) {
    return BHSpawnTileProgramType.wrap(resourceId.unwrap());
  }

  function getAddress(BHSpawnTileProgramType self) internal view returns (address) {
...
using BHSpawnTileProgramLib for BHSpawnTileProgramType global;
using BHSpawnTileProgramLib for CallWrapper global;
using BHSpawnTileProgramLib for RootCallWrapper global;
